...
const styles = {
  ...
  weatherContainer: 'flex flex-col ml-10 max-w-[20ch]',
...
// export default async function Home({ data }: weatherData) {
export default async function Home() {

    const data = await getData() as weatherType
    // console.log(data)
...
//jump to the page of the city
  const handleCityClick = async () => {
    ...
    window.open(`https://unsplash.com/s/photos/${data.name}`, '_blank')
  }
  ...
  return (
    <div>
      ...
      <div className={styles.searchContainer}>
        ...
        <button className={styles.searchBtn} title="search" type="button"   >
          <FaSearch />
        </button>
      </div>
      ...
      <div className={styles.bodyContainer} >
        ...
        <div className={styles.weatherContainer} >
          <button type="button" className={`${styles.weatherTitle}${styles.gradientColor}`} onClick={handleCityClick}>{data.name}</button>
